#include "service_save_file_html.h"



CServiceSaveFileHtmlThread::CServiceSaveFileHtmlThread()
{
}

void CServiceSaveFileHtmlThread::setListDataService(CListDataService *_list_data_service)
{
    m_list_data_service = _list_data_service;
}

void CServiceSaveFileHtmlThread::setTreeWidget                  (QTreeWidget *_tree_widget)
{
    m_tree_widget = _tree_widget;
}

void  CServiceSaveFileHtmlThread::setFile (QFile *_file)
{
    m_file = _file;
}

void CServiceSaveFileHtmlThread::setStop (bool _stop)
{
    m_stop = _stop;
}

void CServiceSaveFileHtmlThread::run()
{
    if (m_file != NULL)
    {
        if (m_file->open(QIODevice::WriteOnly | QIODevice::Text))
        {
            QString chaine = "<html><meta http-equiv=\"content-type\" content=\"text/html; charset=utf-8\" /><title>Données C2A</title><script language='JavaScript'> setTimeout('window.location.reload()',5000); </script><body><center><p>Données du démonstrateur c2a (fichier html)</p><table border='1'>";
            ...
            for (int i=0 ;  i < m_list_data_service->count() && m_stop == false; i++)
            {
                ...
                for (int j=0 ;  j < m_list_data_service->index(i)->count()  && m_stop == false ; j++)
                {
                    ...
                    for (int k = 0 ; k < m_list_data_service->index(i,j).count()  && m_stop == false ; k++)
                    {
                        ...
                        {
                            chaine += "<tr> <td>"+m_list_data_service->index(i,j,k).getStringList().join("</td> <td>")+"</td> </tr>";
                        }
                    } 
                }
            }

            chaine += "</table></center></body></html>";
...
CServiceSaveFileHtml::CServiceSaveFileHtml()
{
    m_thread = new CServiceSaveFileHtmlThread;
    m_start = false;
    createWidget ();
}
...
void CServiceSaveFileHtml::init (CListDataService *_list_data_informations)
{
...
QObject *CServiceSaveFileHtml::getObject()
{
    return this;
}

QString CServiceSaveFileHtml::getNameService()
{
    return "service save file html";
}


void CServiceSaveFileHtml::createWidget ()
{
    ...
    m_line_edit_file_name->setText ("index.html");

    QTextEdit *m_text_edit_description = new QTextEdit;
    m_text_edit_description->setText("Ce service permet d'enregistrer les données périodiquement dans un fichier html qui se situe dans le répertoire de travail. Peut être faudrait t'il sauvegarder les données que si il y a mise à jour de celle-ci. A voir. Pour mettre le fichier sur un serveur ftp il suffit d'activer un service pour envoyer les fichiers sur un serveur ftp. Il faudrait passer par une base de donnée, car des fois il faut rafraichir manuellement la page html car le fichier n'est pas bien copié sur le serveur.");
...
void CServiceSaveFileHtml::initWidget()
{
    m_tree_widget->clear();
...
void CServiceSaveFileHtml::updateWidget()
{
    for (int i=0 ;  i < m_list_data_service->count() ; i++)
    {
        //Je crée l'item parent
...
QWidget * CServiceSaveFileHtml::getWidget()
{
    return m_widget;
}

QString CServiceSaveFileHtml::getDescriptionService()
{
    return "sauver les données dans un fichier";
}


void CServiceSaveFileHtml::start ()
{
...
void CServiceSaveFileHtml::stop()
{
    m_thread->setStop(true);
    m_start = false;
}

void CServiceSaveFileHtml::save (QDomDocument &_dom_document , QDomElement &_dom_element)
{
...
void CServiceSaveFileHtml::load (QDomDocument & , QDomElement &_dom_element)
{
...
void CServiceSaveFileHtml::slotUpdate()
{
    updateWidget();
...
Q_EXPORT_PLUGIN2(serviceSaveFileHtml , CServiceSaveFileHtml)

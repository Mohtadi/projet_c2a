...
void CCuteCom::resizeEvent(QResizeEvent *e)
{
       WRITE_ERROR_LOGGER()
...
/*      m_sz->addArgument("sx");
      ...
      m_sz->start(program , arguments);
      if (! (m_sz->state() == QProcess::NotRunning) )
      {
...
//    cerr<<"while(isRunning)"<<std::endl;
      m_progress->setValue(0);    qDebug("readFromStdout");
...
void CCuteCom::sendDone()
{
       WRITE_ERROR_LOGGER()
   //std::cerr<<"sx exited"<<std::endl;
}

bool CCuteCom::eventFilter(QObject* watched, QEvent *e)
{
       WRITE_ERROR_LOGGER()
...
void CCuteCom::oldCmdClicked(QListWidgetItem* item)
{
       WRITE_ERROR_LOGGER()
   if (item==0)
   {
...
void CCuteCom::prevCmd()
{
       WRITE_ERROR_LOGGER()
   if ((unsigned int)m_oldCmdsLb->count()<=m_cmdBufIndex)
   {
      return;
   }
...
bool CCuteCom::sendString(const QString& s)
{
       WRITE_ERROR_LOGGER()
...
bool CCuteCom::sendByte(char c, unsigned int delay)
{
       WRITE_ERROR_LOGGER()
   if (m_fd==-1)
   {
...
void CCuteCom::enableSettingWidgets(bool enable)
{
   WRITE_ERROR_LOGGER()
...
void CCuteCom::disconnectTTY()
{
   WRITE_ERROR_LOGGER()
   disconnectTTYRestore(m_applyCb->isChecked());
}

void CCuteCom::disconnectTTYRestore(bool restoreSettings)
{
   WRITE_ERROR_LOGGER()
   m_outputTimer.stop();
   m_outputBuffer="";
...
void CCuteCom::setNewOptions(int baudrate, int databits, const QString& parity, const QString& stop, bool softwareHandshake, bool hardwareHandshake)
{
       WRITE_ERROR_LOGGER()
...
void CCuteCom::addOutput(const QString& text)
{
   WRITE_ERROR_LOGGER()
...
void CCuteCom::doOutput()
{
   ...
   emit signalReceiveData (m_outputBuffer);
   m_outputView->append(m_outputBuffer); 
   m_outputBuffer.clear();
}

void CCuteCom::hexOutputClicked(bool /* on */)
{
       WRITE_ERROR_LOGGER()
   addOutput("\n");
   m_hexBytes=0;
...
void CCuteCom::enableLogging(bool on)
{
       WRITE_ERROR_LOGGER()
